...
sub Open {
	...
	unless (defined $Mail::Sender::NO_X_MAILER) {
		...
		print_hdr $s, "X-Mailer" => qq{mailmover\r\n}
	}
...
sub Part {
	...
	my ($description, $ctype, $encoding, $disposition, $content_id, $content_location, $msg, $charset);
	if (ref $_[0] eq 'HASH') {
		my $hash=$_[0];
        ...
        $content_location = $hash->{'content_location'},
		$msg = $hash->{'msg'};
		$charset = $hash->{'charset'};
	} else {
		($description, $ctype, $encoding, $disposition, $content_id, $msg, $content_location) = @_;
	}
...
sub SendFile {
	...
	my ($description, $ctype, $encoding, $disposition, $file, $content_id, @files, $content_location);
	if (ref $_[0] eq 'HASH') {
		my $hash=$_[0];
		$description=$hash->{'description'};
		$ctype=$hash->{'ctype'};
		...
		($description, $ctype, $encoding, $disposition, $file, $content_id, $content_location) = @_;
	}
	return ($self->{'error'}=NOFILE) unless $file;
